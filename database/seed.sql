...
INSERT INTO couriers(user_id, vehicle_plate, online, last_seen, current_latitude, current_longitude, current_address)
SELECT user_id, 'RAB123A', TRUE, NOW(), -1.9700, 30.1100, 'Kigali, Rwanda'
FROM "users" WHERE username='courier1'
ON CONFLICT DO NOTHING;

-- Addresses (owned by users)
INSERT INTO addresses(user_id, district, city, longitude, latitude)
VALUES
...
INSERT INTO shipments(created_at, assigned_at, picked_at, courier_id, customer_id, delivered_at, 
...
VALUES (
  ...
  (SELECT c.courier_id FROM couriers c JOIN "users" u ON u.user_id=c.user_id WHERE u.username='courier1'),
  (SELECT customer_id FROM customers cu JOIN "users" u ON u.user_id=cu.user_id WHERE u.username='alice'),
  NULL,
...
VALUES (
  ...
  NULL,
  (SELECT c.courier_id FROM couriers c JOIN "users" u ON u.user_id=c.user_id WHERE u.username='courier1'),
  (SELECT customer_id FROM customers cu JOIN "users" u ON u.user_id=cu.user_id WHERE u.username='bob'),
  NULL,
...
-- Payments
INSERT INTO payments(shipment_id, method, status, paid_at)
VALUES
 ((SELECT shipment_id FROM shipments ORDER BY shipment_id ASC LIMIT 1), 'momo', 'paid', NOW()),
 ((SELECT shipment_id FROM shipments ORDER BY shipment_id DESC LIMIT 1), 'cash', 'pending', NULL);
